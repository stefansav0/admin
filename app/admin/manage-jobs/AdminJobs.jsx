...
const AdminJobs = () => {
    ...
    const fetchJobs = async () => {
        try {
            ...
            const res = await fetch("https://www.finderight.com/api/jobs", {
                cache: "no-store"
            });
    ...
    const handleDelete = async (slug) => {
        if (!window.confirm("🚨 Are you sure you want to completely delete this job post? This action cannot be undone.")) return;

        try {
            const res = await fetch(`https://www.finderight.com/api/jobs/${slug}`, {
                method: "DELETE",
            });

            if (res.ok) {
                // Remove the deleted job from the UI instantly
        ...
        <div className="max-w-6xl mx-auto p-4 md:p-8">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                <div>
                    ...
                    <p className="text-gray-500 text-sm mt-1">View, edit, and delete published job postings.</p>
                </div>
                <Link
                    href="/admin/add-job"
                ...
                >
                    <span className="mr-2 text-lg leading-none">+</span> Add New Job
                </Link>
            </div>
            ...
            {loading ? (
                ...
                <div className="bg-white border border-gray-200 border-dashed rounded-2xl p-12 text-center shadow-sm">
                    ...
                    <p className="text-gray-500 mb-6">You haven't published any job listings yet.</p>
                    <Link
                        href="/admin/add-job"
                        ...
                        >
                            ...
                            <div className="mb-4 md:mb-0 max-w-2xl">
                                ...
                                <div className="flex flex-wrap items-center gap-3 mt-2 text-sm text-gray-600">
                                    ...
                                    {job.category && (
                                        <span className="bg-blue-50 text-blue-700 border border-blue-100 px-3 py-1 rounded-full font-medium">
                                            🏷️ {job.category}
                                        </span>
                                    )}
                                    <span className="flex items-center text-gray-500">
                                        📅 Created: <span className="ml-1 font-medium text-gray-700">{formatDate(job.createdAt)}</span>
                                    </span>
                                </div>
                            </div>
