...
export default function CreatePaper() {
  ...
  // Standard input changes (for top-level strings)
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    setFormData({ ...formData, [e.target.name]: e.target.value });
  };

  // Handle changes for specific link inputs
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`https://www.finderight.com/api/papers`, {
        method: "POST",
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-gray-200 shadow-sm px-4 sm:px-8 py-4 flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-extrabold text-gray-900 tracking-tight">Create Document</h1>
          <p className="text-sm text-gray-500 font-medium">Publish a new resource to the database</p>
        </div>
        <div className="flex gap-3">
          ...
          <button 
            ...
            disabled={isLoading} 
            className="px-6 py-2.5 text-sm font-bold text-white bg-gradient-to-r from-blue-600 to-indigo-600 rounded-lg hover:from-blue-700 hover:to-indigo-700 transition-all shadow-md disabled:opacity-50 flex items-center gap-2"
          >
            {isLoading ? (
            ...
            <div className="space-y-5">
              <div>
                ...
                <input type="text" name="title" required value={formData.title} onChange={handleTitleChange}
                  ...
                  placeholder="e.g., UPSC Prelims 2023 Paper" />
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 min-h-[600px]">
            ...
            <div className="flex flex-col h-full">
              ...
              <textarea 
                ...
                placeholder="<h2>Write your HTML here...</h2>&#10;<p>Supports standard HTML tags.</p>" 
              />
            </div>
