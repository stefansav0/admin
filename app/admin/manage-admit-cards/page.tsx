...
// Define the structure matching your Mongoose AdmitCard model
type AdmitCard = {
    _id: string;
    slug: string;       // ✅ Crucial for Edit/Delete routing
    title: string;
    conductedby?: string; // Mapped from your schema
    publishDate?: string; // Mapped from your schema
};
...
const AdminAdmitCards = () => {
    ...
    const fetchCards = async () => {
        try {
            const res = await fetch("https://www.finderight.com/api/admit-cards", {
                cache: "no-store"
            });
    ...
    const handleDelete = async (slug: string) => {
        if (!window.confirm("🚨 Are you sure you want to completely delete this admit card? This action cannot be undone.")) return;

        try {
            const res = await fetch(`https://www.finderight.com/api/admit-cards/${slug}`, {
                method: "DELETE",
            });

            if (res.ok) {
                // Remove the deleted card from the UI instantly
                setCards((prev) => prev.filter((item) => item.slug !== slug));
            } else {
                const data = await res.json();
                alert(`❌ Failed to delete: ${data.message}`);
            }
        } catch {
            alert("❌ Error deleting admit card. Please check your connection.");
        }
    };
        ...
        <div className="max-w-6xl mx-auto p-4 md:p-8">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                <div>
                    ...
                    <p className="text-gray-500 text-sm mt-1">View, edit, and delete published admit cards.</p>
                </div>
                <Link
                    href="/admin/add-admit-card"
                    className="inline-flex items-center justify-center bg-blue-600 hover:bg-blue-700 text-white font-bold px-6 py-2.5 rounded-full shadow-md hover:shadow-lg transform hover:-translate-y-0.5 transition-all duration-200"
                >
                    <span className="mr-2 text-lg leading-none">+</span> Add Admit Card
                </Link>
            </div>
            ...
            {loading ? (
                ...
                <div className="bg-white border border-gray-200 border-dashed rounded-2xl p-12 text-center shadow-sm">
                    ...
                    <h2 className="text-xl font-bold text-gray-800 mb-2">No Admit Cards Found</h2>
                    <p className="text-gray-500 mb-6">You haven't published any admit cards yet.</p>
                    <Link
                        href="/admin/add-admit-card"
                    ...
                    {cards.map((card) => (
                        ...
                        >
                            ...
                            <div className="mb-4 md:mb-0 max-w-2xl">
                                <h2 className="font-bold text-lg md:text-xl text-gray-900 group-hover:text-blue-600 transition-colors">
                                    {card.title}
                                </h2>
                                <div className="flex flex-wrap items-center gap-3 mt-2 text-sm text-gray-600">
                                    ...
                                    <span className="flex items-center text-gray-500">
                                        📅 Published: <span className="ml-1 font-medium text-gray-700">{formatDate(card.publishDate)}</span>
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-3 w-full md:w-auto mt-2 md:mt-0 pt-4 md:pt-0 border-t md:border-t-0 border-gray-100">
                                {/* ✅ EDIT LINK: Uses SLUG */}
                                <Link
                                    href={`/admin/edit-admit-card/${card.slug}`}
